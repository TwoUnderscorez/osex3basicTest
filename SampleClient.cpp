...
TEST(MattanTests, errorMessageTest)
{
	...
	ASSERT_THAT(output, ::testing::MatchesRegex("system error: .*\n"));
}
...
TEST(MattanTests, progressTest)
{
	...
	{
		printf("\r%c : Running iter %d out of %d (If the spinner stops spinning you have a deadlock)", spinner[i % 4], i, REPEATS - 1);
		testing::internal::CaptureStdout();
...
TEST(MattanTests, deadlockTest)
{
	...
	{
		printf("\r%c : If the spinner stops spinning you have a deadlock in iter %d (out of %d)", spinner[i % 4], i, DEADLOCK_REPEATS - 1);
		testing::internal::CaptureStdout();
		// std::cout << "repetition #" << i << std::endl;
		CounterClient client;
		auto s1 = new VString("This string is full of characters");
		...
		while (state.stage != REDUCE_STAGE || state.percentage != 100.0)
		{
			...
			getJobState(job, &state);
			testing::internal::GetCapturedStdout();
			printf("\r%c : If the spinner stops spinning you have a deadlock in iter %d (out of %d) while in state(%d,%f)", spinner[i % 4], i, DEADLOCK_REPEATS - 1, state.stage, state.percentage);
			testing::internal::CaptureStdout();
		}
		closeJobHandle(job);
		testing::internal::GetCapturedStdout();
...
void randbody(int iterations)
{
	...
	{
		printf("\r%c : Running iter %d out of %d (If the spinner stops spinning you have a deadlock)", spinner[i % 4], i, iterations - 1);
		testing::internal::CaptureStdout();
			...
			if (jobs[j] != nullptr)
			{
				...
				if (!(state.stage == REDUCE_STAGE && last_state.percentage == 100.0))
				{
					if (last_state.stage != state.stage || last_state.percentage != state.percentage)
					{
						...
						{
							std::cout << testing::internal::GetCapturedStdout();
							FAIL() << "Invalid percentage(not in 0-100): " << "Current stage:" << state.stage << " " << state.percentage << "%" << std::endl
						...
						{
							std::cout << testing::internal::GetCapturedStdout();
							FAIL() << "Bad percentage(smaller than previous percentage at same stage): " << "Current stage:" << state.stage << " " << state.percentage << "%" << std::endl
								   << "Previous stage:" << last_state.stage << " " << last_state.percentage << "%"
								   << std::endl;
						}
						if (last_state.stage > state.stage)
						{
							std::cout << testing::internal::GetCapturedStdout();
							FAIL() << "Bad stage: " << "Current stage:" << state.stage << " " << state.percentage << "%" << std::endl
								   << "Previous stage:" << last_state.stage << " " << last_state.percentage << "%"
								   << std::endl;
						}
					}
